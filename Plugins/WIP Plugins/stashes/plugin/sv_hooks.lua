...
function PLUGIN:HasItem(player, itemTable)
	for k, v in pairs (ents.FindByClass("cw_stash")) do
		local stash = player:GetCharacterData("stashItems"..tostring(v:EntIndex()));
...
function PLUGIN:PlayerRestoreCharacterData(player, data)
	...
	for k, v in pairs (ents.FindByClass("cw_stash")) do
		data["stashItems"..tostring(v:EntIndex())] = data["stashItems"..tostring(v:EntIndex())] or {};
		data["stashCash"..tostring(v:EntIndex())] = data["stashCash"..tostring(v:EntIndex())] or 0;
	end;
	...
	for k, v in pairs( data["stashItems"] ) do
		...
		if (!itemTable) then
			PLUGIN:PlayerHasUnknownInventoryItem(player, data["stashItems"], k, v);
