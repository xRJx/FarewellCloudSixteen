										...
										if (IsValid(self) and IsValid(entity)) then
											...
											entity:CallOnRemove("CreatePayroll", function()
												if (IsValid(activator)) then
													local itemTable = Clockwork.item:CreateInstance("payroll");
											...
											if (IsValid(activator) and !force) then
												if (activator:GetCharacterData("customclass") == "Civil Worker's Union") then
													self:ActivatePayroll(activator, 8, true);
												end;
											end;
	...
	if (activator:IsPlayer() and activator:GetEyeTraceNoCursor().Entity == self) then
		...
		if (!self.nextUse or curTime >= self.nextUse) then
			if (Schema:PlayerIsCombine(activator)) then
				if (!self:IsLocked() and unixTime >= activator:GetCharacterData("nextpayroll", 0)) then
					if (!self.nextActivatePayroll or curTime >= self.nextActivatePayroll) then
						self:ActivatePayroll(activator);
						
						activator:SetCharacterData("nextpayroll", unixTime + Clockwork.config:Get("wages_interval"):Get() * 10);
					end;
				else
