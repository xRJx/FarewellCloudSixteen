...
namespace Methods
{
	...
	static int compile(lua_State* state, const char* code, const char* id)
	{
	...
	static int GetConVarString(lua_State* state)
	{
		/*GetGlobalField(state, "GetConVarString");
	...
	static int RunString(lua_State* state, const char* code)
	{
	 ...
	 static int assert(lua_State* state, bool Condition, const char* Message = "assertion failed!")
	 {
			...
			namespace debug
			{
				static int getFuncInfo(lua_State* state)
				{
					int RS = GetGlobalField(state, "RunString");
					GetGlobalField(state, "debug");
					LUA->GetField(-1, "getinfo");
					LUA->ReferencePush(RS);
					LUA->Call(1, 1);
					LUA->GetField(-1, "func");
					return RS;
				}

				static int getupvalue(lua_State* state, const char* functionName, const char* index)
				{
						GetGlobalField(state, "debug");
						LUA->GetField(-1, "getupvalue");
						LUA->PushString(functionName);
