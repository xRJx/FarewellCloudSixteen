...
AccessorFunc(ENT, "shouldDissolve", "Dissolve");
...
function ENT:DissolveEntity(entity)
    if ( self:GetDissolve() ) then
        if ( !self.isDissolving and !table.HasValue( blacklist, entity:GetClass() ) ) then
    ...
    if ( (entity.nextFFTouch or 0) < CurTime() ) then
        if ( entity:IsPlayer() or entity:IsNPC() ) then
            if ( entity:IsPlayer() ) then
                if (openAura.player:GetFaction(entity) == FACTION_CITIZEN) then
...
function ENT:Use(activator, caller)
    if ( IsValid(activator) and activator:IsPlayer() and openAura.schema:PlayerIsCombine(activator) ) then
