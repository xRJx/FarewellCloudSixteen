...
function COMMAND:OnRun(player, arguments)
	...
	openAura:StartDataStream( nil, "SurfaceTextAdd", data );
...
openAura.command:Register(COMMAND, "TextAdd");
...
function COMMAND:OnRun(player, arguments)
	...
	for k, v in pairs(PLUGIN.surfaceTexts) do
		if (v.position:Distance(position) <= 256) then
			openAura:StartDataStream( nil, "SurfaceTextRemove", v.position );
...
openAura.command:Register(COMMAND, "TextRemove");