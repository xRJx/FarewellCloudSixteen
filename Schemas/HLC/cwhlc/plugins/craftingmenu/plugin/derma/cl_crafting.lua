...
function PANEL:Rebuild()
	...
	for k, v in pairs(Clockwork.item:GetAll()) do
		if ( (!v("isBaseItem") and v("crafting")) and Clockwork.kernel:HasObjectAccess(Clockwork.Client, v)) then
			if (PLUGIN:PlayerCanSeeCraftingItem(v)) then
...
vgui.Register("cwCraftingCustom", PANEL, "DPanel");
...
function PANEL:Init()
	...
	PLUGIN:PlayerAdjustCraftingItemTable(self.itemTable);
	...
	function self.spawnIcon.DoClick(spawnIcon)
		Clockwork.kernel:RunCommand(
			"CraftItem", self.itemTable("uniqueID")
		);
	end;
...
-- Called each frame.
function PANEL:Think()
	self.spawnIcon:SetMarkupToolTip(Clockwork.item:GetMarkupToolTip(self.itemTable, false));
