...
function COMMAND:OnRun(player, arguments)
	...
	Clockwork.plugin:Call("PlayerAdjustCraftItemTable", player, itemTable);
	
	if (!Clockwork.plugin:Call("PlayerCanCraftItem", player, itemTable)) then
		return false;
	end;
	...
	if (itemTable.AllowCraft) then
		if (itemTable:AllowCraft(player, itemTable)) then
			...
			Clockwork.plugin:Call("PlayerCraftItem", player, itemTable, entity);
